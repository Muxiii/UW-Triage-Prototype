...
export function getStore() {
  if (!storeSingleton) {
    if (isCloudRuntime() && !useSupabase()) {
      const host = process.env.VERCEL ? 'Vercel' : process.env.RENDER ? 'Render' : 'this host';
      throw new Error(
        `Cloud deploy on ${host} requires Supabase: set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY. `
        + (process.env.VERCEL
          ? 'Vercel should only host static files from public/ — deploy the API on Render instead of running server.mjs on Vercel.'
